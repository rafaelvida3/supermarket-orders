<template>
  <div class="max-w-5xl mx-auto p-4 sm:p-6 bg-white dark:bg-gray-900 shadow rounded-lg transition-colors duration-300">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
      <div>
        <h1 class="text-xl sm:text-2xl font-semibold text-gray-800 dark:text-gray-100">Estoque</h1>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
      <article class="border rounded-md p-4 dark:border-gray-700 dark:bg-gray-800">
        <p class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Produtos</p>
        <p class="text-2xl font-semibold text-gray-900 dark:text-gray-100 mt-2">{{ summary.totalProducts }}</p>
      </article>

      <article class="border rounded-md p-4 dark:border-gray-700 dark:bg-gray-800">
        <p class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Baixo estoque</p>
        <p class="text-2xl font-semibold text-amber-600 dark:text-amber-400 mt-2">{{ summary.lowStockProducts }}</p>
      </article>

      <article class="border rounded-md p-4 dark:border-gray-700 dark:bg-gray-800">
        <p class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Sem estoque</p>
        <p class="text-2xl font-semibold text-red-600 dark:text-red-400 mt-2">{{ summary.outOfStockProducts }}</p>
      </article>
    </div>
    ...
    <div v-if="!loading && products.length > 0" class="md:hidden space-y-3">
      ...
      >
        <div class="flex items-start justify-between gap-3">
          <div>
            <p class="text-base font-semibold text-gray-900 dark:text-gray-100 break-words">
              {{ product.name }}
            </p>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
          ...
          <Tag :severity="getStockSeverity(product.qty_stock)" :value="getStockLabel(product.qty_stock)" rounded />
        </div>

        <div class="mt-4 grid grid-cols-2 gap-3">
          <div>
            <p class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">ID</p>
            <p class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ product.id }}</p>
          </div>

          <div>
            <p class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Quantidade</p>
            <p class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ product.qty_stock }}</p>
          </div>
        </div>
      </article>
    </div>
    ...
    <div class="hidden md:block overflow-x-auto rounded-md border dark:border-gray-700">
      ...
      >
        ...
        <Column field="qty_stock" sortable>
          <template #header>
            <span class="font-bold dark:text-gray-100 whitespace-nowrap">Quantidade</span>
          </template>
        </Column>
